...
- Add R code for predictiveMargins
- add Sönning et al. 2025 to references
...
In such cases, different statistical techniques may be used to adjust for known mismatches between sample and population. This blog post is a companion to Sönning et al. (2025), and it describes how to implement two approaches: The use of sampling weights during model fitting, and the use of post-stratification weights when post-processing model-based predictions. 
...
library(lattice)
library(party)
library(predictiveMargins) # for forming predictive margins
library(pdp)
library(kableExtra)
library(gtsummary)
...
We will use the same illustrative example as Sönning et al. (2025): A hypothetical diachronic study with a binary response variable. Data are available on male and female speakers in two age groups (older and younger), and there are 200 speakers in total; each speaker contributes only one token to the data. Interest centers on how the usage rate, expressed as a percentage, varies with Gender and Age. 
...
d <- data.frame(
  gender = rep(c("female", "male", "female", "male"), c(20, 100, 20, 60)),
  age = rep(c("old", "young"), c(120, 80)),
  response = c(rep(c(0,1), c(17, 3)),
               rep(c(0,1), c(95, 5)),
               rep(c(0,1), c(14, 6)),
               rep(c(0,1), c(54, 6))),
  sampling_weight = rep(c(1/(20/200), 1/(100/200), 1/(20/200), 1/(60/200)), c(20, 100, 20, 60))
)
...
#| fig-cap: "Data for our illustrative (hypothetical) example: percentages (a) for the four subgroups in the data and (b) averages over various subgroups."

p1 <- xyplot(
  1~1, type = "n", xlim = c(.2, 2.8), ylim = c(0, 32),
  par.settings = my_settings, axis = axis_L,
  ylab = "Percentage", xlab = NULL,
  scales = list(x = list(at = 1:2, labels = c("older", "younger")),
                y = list(at = c(0, 10, 20, 30))),
       panel = function(x,y){
         panel.text(x = .9, y = c(5, 15), label = c("5%", "15%"), cex=.8, adj = 1)
         panel.text(x = 2.1, y = c(10, 30), label = c("10%", "30%"), cex=.8, adj = 0)
         
         panel.points(x = 1:2, y = c(15, 30), pch = 21, type = "l", lty = "22")
         panel.points(x = 1:2, y = c(5, 10), pch = 19, type = "l")
         
         
         panel.points(x = 1:2, y = c(15, 30), pch = 21, fill = "white")
         panel.points(x = 1:2, y = c(5, 10), pch = 19)
         
         panel.text(x = c(1.6, 1.2), y = c(4, 27), label = c("male", "female"), cex = .8)
...
p2 <- xyplot(
  ...
  scales = list(x = list(at = 1:2, labels = NULL),
       ...
       panel = function(x,y){
         panel.text(x = .9, y = c(5, 15), label = c("5%", "15%"), cex=.8, adj = 1, col = "grey")
         panel.text(x = 2.1, y = c(10, 30), label = c("10%", "30%"), cex=.8, adj = 0, col = "grey")
         
         panel.points(x = 1:2, y = c(15, 30), pch = 21, type = "l", lty = "22", col = "grey")
         panel.points(x = 1:2, y = c(5, 10), pch = 19, type = "l", col = "grey")
         
         panel.points(x = 1:2, y = c(15, 30), pch = 21, fill = "white", col = "grey")
         panel.points(x = 1:2, y = c(5, 10), pch = 19, col = "grey")
         
         panel.text(x = c(1.5, 1.5), y = c(5.5, 24.5), label = c("(male) ", "(female) "), cex = .8)
         
         panel.text(x = .9, y = 10, label = "(older) 10%", cex=.8, adj = 1)
         panel.text(x = 2.1, y = 20, label = "20% (younger)", cex=.8, adj = 0)
         
         panel.text(x = 1.5, y =  7.5, label = "7.5%", cex=.8)
         panel.text(x = 1.5, y = 22.5, label = "22.5%", cex=.8)
         panel.text(x = 1.5, y = 15, label = "15%", fontface = "bold", cex = .8)
...
#| code-fold: true
#| code-summary: "Draw figure"
#| fig-cap: "Data for our illustrative (hypothetical) example: proportional representation of the four subgroups."
#| classes: preview-image

p1 <- xyplot(
  1~1, type = "n", xlim = c(0.1, 2.7), ylim = c(0, 32),
  par.settings = my_settings, axis = axis_L,
  ylab = "Percentage",xlab = NULL,
  scales = list(x = list(at = 1:2, labels = c("older", "younger")),
                y = list(at = c(0, 10, 20, 30))),
       panel = function(x,y){
         ...
         panel.text(x = .9, y = 18.5, label = "n = 20", adj = 1, cex = .8, col = "grey30")
         panel.text(x = .9, y = 8.5, label = "n = 100", adj = 1, cex = .8, col = "grey30")
         
         panel.text(x = 2.1, y = 33.5, label = "n = 20", adj = 0, cex = .8, col = "grey30")
         panel.text(x = 2.1, y = 13.5, label = "n = 60", adj = 0, cex = .8, col = "grey30")
       })

print(p1, position = c(0,0,1,.95))
```

The distribution of the variables Gender and Age in our sample do not reflect that in the population of interest, which includes (nearly) the same number of male and female speakers, and, we will assume, also equal numbers of younger and older speakers. 

The imbalance in our data will have no consequences for the estimation of the quantities in (i). However, once we average out a variable, we must decide how much weight to give to each of its levels. The summary percentages in panel (b) of @fig-data (quite reasonably) assume that the two age groups and the two genders should be weighted equally. By taking a simple (i.e. unweighted) average over male and female speakers, for instance, we have manually adjusted for the lack of representativeness in our data. 

In the next section, we will look at how adjustments of this type may be accomplished in a multifactorial analysis. We primarily focus on regression and – to keep things straightforward – we will apply a linear probability model to the data. This means that we do not analyze them using logistic regression but instead use an ordinary linear regression model. This mainly serves to keep the present discussion focused on the aspect of main concern – we would usually analyze binary outcome variables using logistic regression.  We should note that the simplified analysis strategy does not compromise the insights that emerge, which apply to any form of regression analysis.
...
For comparison, we also fit a standard linear regression model with the function `lm()`, which does not take into account sampling weights:
...
We note that the two models return identical coefficient estimates, with the standard errors produced by `svyglm()` being somewhat larger.

Let us now turn to our model-based estimates. We can obtain the population-level percentage (iv) and its standard error with the following code, which returns an estimate of 15% (the same as in panel (b) of @fig-data) with a standard error of 3.5:
...
The `{predictiveMargins}` package [@Grafmiller_Soenning2022] allows us to modify the weighting scheme and thereby obtain the desired equal-handed treatment of male and female speakers.
