...
`A2Rplot_modified` <- function(
	...
	widths <- c(1, 9)
	heights <- c(1-space.labels -.1, space.labels, .1)*10
	if (!show.labels) {
		matlayout <- matrix(c(2, 4, 1, 3), nc = 2, nr = 2)
		widths <- c(1, 9)
		heights <- c(1-space.labels, space.labels)*10
	}
	if (!missing(fact.sup)) {
		heights <- c(1-space.labels -.1, space.labels, .1)*10
	}
	...
	if (show.labels) {
		...
		if (is.null(members)) {
			if(is.null(new.labels)){
				plot(0, type = "n", xlim = c(0.5, n.indiv + .5), ylim = c(0, 1), xaxs = "i", yaxs = "i", axes = FALSE, xlab = "", ylab = "")
				text(1:n.indiv, 1 - distance.labels, obs.labels, col = col.down[groups.o], adj = c(1, 0.5))			
			}
			else{
				plot(0, type = "n", xlim = c(0.5, n.indiv + .5), ylim = c(0, 1), xaxs = "i", yaxs = "i", axes = FALSE, xlab = "", ylab = "")
				text(1:n.indiv, 1 - distance.labels, new.labels, col = col.down[groups.o], adj = c(1, 0.5))							
			}

		}
		else {
			if(is.null(new.labels)){
				plot(0, type = "n", xlim = c(0.5, sum(members) + .5), ylim = c(0, 1), xaxs = "i", yaxs = "i", axes = FALSE, xlab = "", ylab = "")
				xo <- members[x$order]
				text(cumsum(xo) - xo / 2, 1 - distance.labels, obs.labels, col = col.down[groups.o], adj = c(1, 0.5))			
			}
			else{
				plot(0, type = "n", xlim = c(0.5, sum(members) + .5), ylim = c(0, 1), xaxs = "i", yaxs = "i", axes = FALSE, xlab = "", ylab = "")
				xo <- members[x$order]
				text(cumsum(xo) - xo / 2, 1 - distance.labels, new.labels, col = col.down[groups.o], adj = c(1, 0.5))							
			}
